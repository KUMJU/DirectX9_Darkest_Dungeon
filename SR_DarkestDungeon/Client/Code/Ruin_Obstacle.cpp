...
CRuin_Obstacle::CRuin_Obstacle(const CRuin_Obstacle& rhs)
	: CInteractionObj(rhs)
{
}

CRuin_Obstacle::~CRuin_Obstacle()
{
}
...
_int CRuin_Obstacle::UpdateGameObject(const _float& fTimeDelta)
{
	...
	if (m_eCurAnimState != m_ePrevAnimState)
	{
		...
		{
		case EState::IDLE:
			m_pAnimatorCom->SetAnimKey(L"Ruin_Obstacles_Remains", 0.05f);
			//m_pTransformCom->SetScale(WEALD_PATHSIZEX / 3.f, WEALD_PATHSIZEX / 3.f, 1.f);
			break;
		case EState::ACTIVE:
			m_pAnimatorCom->SetAnimKey(L"Ruin_Obstacles", 0.04f);
			//m_pTransformCom->SetScale(WEALD_PATHSIZEX / 3.f, WEALD_PATHSIZEX / 3.f, 1.f);
			break;
		case EState::FINISH:
...
void CRuin_Obstacle::LateUpdateGameObject()
{
	__super::LateUpdateGameObject();
}

void CRuin_Obstacle::RenderGameObject()
{
...
void CRuin_Obstacle::AddComponent()
{
...
void CRuin_Obstacle::GetInteractionKey(const _float& fTimeDelta)
{
	// 인터랙션 키 UI 보여주기
	// ----------수정 필요------------
...
void CRuin_Obstacle::Interaction()
{
}

void CRuin_Obstacle::FinishInteraction()
{
...
void CRuin_Obstacle::ChangeTexture()
{
	m_ePrevAnimState = m_eCurAnimState;
	m_eCurAnimState = EState::ACTIVE;
}

_bool CRuin_Obstacle::IsFinish()
{
	//// 키 입력받기
