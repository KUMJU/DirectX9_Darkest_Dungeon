...
CAlienMob::CAlienMob(LPDIRECT3DDEVICE9 pGraphicDev)
	: CMonster(pGraphicDev)
{
}

CAlienMob::CAlienMob(const CCreature& rhs)
	: CMonster(rhs)
{
}
...
HRESULT CAlienMob::ReadyGameObject()
{
	...
	int Skill1_Dot[2] = { 10,2 };
	_bool	bArrAttack1[8] = { 0, 1, 0, 0, 0, 0, 0, 0 };
...
_int CAlienMob::UpdateGameObject(const _float& fTimeDelta)
{
	...
	if (m_eCurAnimState != m_ePrevAnimState)
	{
		...
		case EAnimState::BESHOT:
			m_pTextureCom->SetAnimKey(L"Mob_Death", 0.8f);
			m_pTransformCom->SetScale(2.5f * 1.f / 1.f, 2.5f * 1.f / 1.f, 1.f * 1.f / 1.f);
			m_pTransformCom->SetPosition(m_vPos.x, 2.5f * 1.f / 1.f, m_vPos.z);
			break;
		case EAnimState::SKILL1:
			m_pTextureCom->SetAnimKey(L"Mob_Shooting", 0.06f);
			m_pTransformCom->SetScale(2.5f * 1.f / 1.f, 2.5f * 1.f / 1.f, 1.f * 1.f / 1.f);
			m_pTransformCom->SetPosition(m_vPos.x, 2.5f * 1.f / 1.f, m_vPos.z);
			break;
...
void CAlienMob::LateUpdateGameObject()
{
	__super::LateUpdateGameObject();
}

void CAlienMob::RenderGameObject()
{
