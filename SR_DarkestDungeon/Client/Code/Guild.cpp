...
HRESULT CGuild::ReadyGameObject()
{
	...
	// GameObject
	{
		// NPC
		{
			shared_ptr<CGameObject> m_pNPC = make_shared<CGuildNPC>(m_pGraphicDev);
		...
		//// 수련인형
		//{
		//	shared_ptr<CGameObject> m_pSandBack = make_shared<CSandBack>(m_pGraphicDev);
		//	m_pSandBack->SetScale({ 6.f, 5.f, 1.f });
		//	m_pSandBack->SetPos({ m_vPos.x + VILLAGE_TILESIZE * 2 + 3.f, m_vPos.y + m_pSandBack->GetScale().y / 2 + 1, m_vPos.z + VILLAGE_TILESIZE * 2 + 5.f });
