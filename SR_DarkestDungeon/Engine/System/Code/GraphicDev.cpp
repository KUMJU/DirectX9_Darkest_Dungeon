...
HRESULT CGraphicDev::Ready_GraphicDev(HWND hWnd, WINMODE eMode, const _uint& iSizeX, const _uint& iSizeY, CGraphicDev** ppGraphicDev)
{
	...
	if (E_FAIL == m_pSDK->CreateDevice(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, hWnd, dwFlag, &d3dpp, &m_pGraphicDev))
		return E_FAIL;
...
//	if(dwRefCnt = m_pGraphicDev->Release())
	//	MSG_BOX("m_pGraphicDev Release Failed");

//	if (dwRefCnt = m_pSDK->Release())
